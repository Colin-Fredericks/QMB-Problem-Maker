function [answers,correctness,path_str,file_name,explanations] = path3()
...
right_answers = {['$myImage = imread(''' path_str '/' file_name ''');/$'], ...
                 ['$uiopen(''' path_str '/' file_name ''')/$'], ...
                 'Dragging the image from Finder/File Explorer into the Command Window', ...
                 ['Clicking the "Open" button on Matlab toolstrip (top left under the Home tab), navigating to "' path_str '", and selecting the file'], ...
   };

right_explanations = { 'Yes. You can always type the full file location as a string with $imread/$', ...
    'Yes. The $uiopen/$ function will open a window that will allow you to load the image', ...
    ['Yes. This is equivalent to typing $uiopen(''' path_str '/' file_name ''')/$ into the command line'], ...   
    'Yes. Many of the things we teach can also be accomplished using the graphical interface, but they cannot be automated. You wouldn''t want to open 1000 image files this way.'};
...
wrong_answers = {['Yelling loudly at the computer until it does what you want'], ...
                 ...
                 ['$uiopen(' path_str '/' file_name ')/$'], ...
                 ['$myImage = imread(' path_str '/' file_name ');/$'], ...
                 ['$myImage = ''' path_str '/' file_name ''';/$']};
wrong_explanations = {['No. Matlab is not intimidated by your threats.'], ...
                      ...
                      'No. You must enclose the path and file names in single quotes to create a string', ...
                      'No. This will create a string variable that has the full file name. It will not load the image'};
