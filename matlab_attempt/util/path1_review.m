function [answers,correctness,path_str,file_name,explanations] = path1_review()
% Function path1_review() - QMB problem path1_review()
%
%   [answers,correctness,explanations,path_str,file_name] = path1_review()
%       Generates the answers for QMB problem path1_review()
%
%
...
right_answers = {['Change your current directory to "' path_str '"'], ...
                 ...
                 ['Type $readtable(''' path_str '/' file_name ''')/$ instead'], ...
...
wrong_explanations = {['No. Matlab does not search folders recursively. You need to add the lowest level folder to the path. If you use the Set Path interface, you can select the "Add with subfolders" option and select "' root '". Then, every subfolder in "' root '" will be added to the path.'], ...
                      ...
                      'No. The function $readtable/$ is a built-in function. If you used an undefined function, then the error message would say something like "Undefined function or variable" instead of "File does not exist', ...
